...
MM_StandardAccessBarrier::preObjectRead(J9VMThread *vmThread, J9Object *srcObject, fj9object_t *srcAddress)
{
	...
	if (NULL != _scavenger) {
		...
		if (_scavenger->isObjectInEvacuateMemory(object)) {
			Assert_GC_true_with_message3(env, _scavenger->isConcurrentCycleInProgress(),
					"CS not in progress, j9vmthread %llx found a object in Survivor: slot %llx object %llx\n", env->getLanguageVMThread(), srcAddress, object);
			Assert_MM_true(_scavenger->isMutatorThreadInSyncWithCycle(env));
