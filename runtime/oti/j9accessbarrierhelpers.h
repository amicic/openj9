...
#if (defined(__GNUC__) && (defined(J9X86) || defined(S390)))
__attribute__ ((noinline))
#endif /* (defined(__GNUC__) && defined(J9X86)) */
static UDATA j9javaArraylet_BA(J9VMThread *vmThread, J9IndexableObject *array, U_32 *index, U_8 elementSize)
{
	UDATA baseAddress = 0;
	...
	if (J9ISCONTIGUOUSARRAY(vmThread, array)) {
		...
		U_32 leafIndex = *index / elementsPerLeaf;
		*index = *index % elementsPerLeaf;
...
VMINLINE static UDATA j9javaArray_BA(J9VMThread *vmThread, J9IndexableObject *array, U_32 *index, U_8 elementSize)
{
	...
	if (J9IndexableObjectLayout_NoDataAddr_NoArraylet == vmThread->indexableObjectLayout) {
		...
		baseAddress = j9javaArraylet_BA(vmThread, array, index, elementSize);
	}
...
VMINLINE static elemType *j9javaArray_##elemType##_EA(J9VMThread *vmThread, J9IndexableObject *array, U_32 index)	\
{																													\
	UDATA baseAddress = j9javaArray_BA(vmThread, array, &index, (U_8)sizeof(elemType));								\
	/* intentionally inlining this to treat sizeof value as an immediate */											\
	return (elemType *)(baseAddress + (UDATA)index * sizeof(elemType));												\
} 																													\
