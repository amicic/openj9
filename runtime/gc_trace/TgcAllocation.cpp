...
tgcAllocationPrintCumulativeStats(OMR_VMThread *omrVMThread)
{
	...
	tgcExtensions->printf("Alloc Contention Count:        %12zu\n", allocStats->_allocContentionCount);
...
tgcAllocationPrintPerThreadStats(OMR_VMThread *omrVMThread)
{
...
#if defined(J9VM_GC_THREAD_LOCAL_HEAP)
	tgcExtensions->printf("   JVM thread ID |                                                                             name | contention | refresh/remaining cache size | obj alloc count/bytes | TLH fresh count/bytes | TLH discards\n");
#else /* defined(J9VM_GC_THREAD_LOCAL_HEAP) */
	tgcExtensions->printf("   JVM thread ID |                                                                             name | contention | refresh/remaining cache size | obj alloc count/bytes\n");
#endif /* defined(J9VM_GC_THREAD_LOCAL_HEAP) */
	...
	while ((walkOmrVMThread = threadListIterator.nextOMRVMThread()) != NULL) {
		...
		tgcExtensions->printf("%8p | %80s | %10zu | %14zu %13zu | %8zu %12zu", walkOmrVMThread->_language_vmthread, getOMRVMThreadName(walkOmrVMThread),
				walkStats->_allocContentionCount, walkInterface->getRefreshCacheSize(false), walkInterface->getRemainingCacheSize(false), walkStats->_allocationCount,  walkStats->_allocationBytes);
#if defined(J9VM_GC_THREAD_LOCAL_HEAP)
